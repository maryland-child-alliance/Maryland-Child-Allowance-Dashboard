# Maryland Child Allowance Dashboard guide installation
...
activate Maryland-Child-Allowance-Dashboard
```
...
To move beyond notebook prototyping, all reusable code should go into the `maryland_child_allowance_dashboard/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `maryland_child_allowance_dashboard` folder and use the modules inside your notebooks :
...
```python
from maryland_child_allowance_dashboard.utils.paths import data_dir
data_dir()
```
